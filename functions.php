//шоркод с курсом евро
add_shortcode( 'cureuro', 'cureuro_func' );

function cureuro_func($atts){
	...
	foreach ($xml->Valute as $one) {
		if ($one["ID"] == 'R01239') { //выбираем евро по коду цб
			$koeficient1 = round(str_replace(',','.',$one->Value), 2); //ее значение
			$number_percent = $koeficient1 / 100 * $percent;
			$procent_plus = $koeficient1 + $number_percent;
			$procent_plus_r =round($procent_plus, 2);
